...
Unified workflow for testing flow installation across multiple platforms, supporting both manual and programmatic execution.

## Manual Testing

### 1. Test from PyPI
Tests published flow packages from PyPI across all platforms.
...
gh workflow run cross-platform-test.yml \
  ...
  -f flow-version="1.0.18"
```

### 2. Test from Source
Builds and tests flow from current branch source code using release-like dependency resolution (transforms workspace dependencies to published packages for testing parity).
...
1. **Package Installation**: `uv pip install flow` (PyPI) or local wheel installation
2. **Dependencies**: Additional packages like `openai` for full functionality
3. **CLI Help**: `flow --help`
4. **Server Startup**: `flow run --backend-only` with `/health_check` endpoint validation
5. **Python Import**: `import flow`
...
|------------|----------------|--------------|----------|---------|
| **Manual/Source** | Workspace (`flow-base = { workspace = true }`) | 162 packages | ❌ Not included | ✅ Success |
| **Nightly/Release** | Published (`flow-base-nightly==0.5.0.dev21`) | 420 packages | ✅ Included | ❌ Compilation fails |
...
2. **Nightly builds** modify dependencies via `scripts/ci/update_uv_dependency.py`:
   - Changes: `flow-base~=0.5.0` → `flow-base-nightly==0.5.0.dev21`
