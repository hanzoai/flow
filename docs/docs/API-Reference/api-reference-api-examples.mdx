---
title: Get started with the Hanzo Flow API
slug: /api-reference-api-examples
---
import CodeSnippet from '@site/src/components/CodeSnippet';
import exampleApiReferenceApiExamplesFormHanzo FlowApiRequests from '!!raw-loader!@site/docs/API-Reference/curl-examples/api-reference-api-examples/form-flow-api-requests.sh';
...
import examplePythonApiReferenceApiExamplesFormHanzo FlowApiRequests from '!!raw-loader!@site/docs/API-Reference/python-examples/api-reference-api-examples/form-flow-api-requests.py';
import exampleJavascriptApiReferenceApiExamplesFormHanzo FlowApiRequests from '!!raw-loader!@site/docs/API-Reference/javascript-examples/api-reference-api-examples/form-flow-api-requests.js';
...
You can use the Hanzo Flow API for programmatic interactions with Hanzo Flow, such as the following:

* Create and edit flows, including file management for flows.
* Develop applications that use your flows.
* Develop custom components.
* Build Hanzo Flow as a dependency of a larger application, codebase, or service.
* Contribute to the overall Hanzo Flow codebase.

To view and test all available endpoints, you can access the Hanzo Flow API's OpenAPI specification at your Hanzo Flow deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the Hanzo Flow API in a script, see the [Hanzo Flow quickstart](/get-started-quickstart).
...
## Form Hanzo Flow API requests

While individual options vary by endpoint, all Hanzo Flow API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a Hanzo Flow API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

<Tabs>
<TabItem value="Python" label="Python" default>

<CodeSnippet source={examplePythonApiReferenceApiExamplesFormHanzo FlowApiRequests} language="python" />

</TabItem>
<TabItem value="JavaScript" label="JavaScript">

<CodeSnippet source={exampleJavascriptApiReferenceApiExamplesFormHanzo FlowApiRequests} language="javascript" />

</TabItem>
<TabItem value="curl" label="curl">

<CodeSnippet source={exampleApiReferenceApiExamplesFormHanzo FlowApiRequests} language="bash" />
...
By default, local deployments serve the Hanzo Flow API at `http://localhost:7860/api`.

Remotely hosted Hanzo Flow deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:FLOW_PORT/api`.

You can configure the Hanzo Flow port number in the `FLOW_PORT` [environment variable](/environment-variables).

* `https://UUID.ngrok.app/api`
* `http://IP_OR_DNS/api`
* `http://IP_OR_DNS:FLOW_PORT/api`

### Authentication

In Hanzo Flow versions 1.5 and later, most API endpoints require authentication with a Hanzo Flow API key in either an `x-api-key` header or query parameter.
...
Hanzo Flow API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The Hanzo Flow API serves `/v1` and `/v2` endpoints.
...
Commonly used values in Hanzo Flow API requests include your [Hanzo Flow server URL](#base-url), [Hanzo Flow API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some Hanzo Flow API requests

Once you have your Hanzo Flow server URL, try calling these endpoints that return Hanzo Flow metadata.

### Health check

Returns the health status of the Hanzo Flow database and chat services:
...
Hanzo Flow provides an additional `GET /health` endpoint.
This endpoint is served by uvicorn before Hanzo Flow is fully initialized, so it's not reliable for checking Hanzo Flow service health.

### Get version

Returns the current Hanzo Flow API version:
...
Returns configuration details for your Hanzo Flow deployment.
Requires a [Hanzo Flow API key](/api-keys-and-authentication).
...
Returns a dictionary of all Hanzo Flow components.
Requires a [Hanzo Flow API key](/api-keys-and-authentication).
...
Because you can run Hanzo Flow as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the Hanzo Flow database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the Hanzo Flow codebase, you won't directly call most of the Hanzo Flow endpoints.
...
The following endpoints are useful for developing applications with Hanzo Flow and administering Hanzo Flow deployments with one or more users.
...
* Deployment details:
  * GET `/v1/version`: Return Hanzo Flow version. See [Get version](/api-reference-api-examples#get-version).
...
* Custom components: You might use these endpoints when developing custom Hanzo Flow components for your own use or to share with the Hanzo Flow community:
  * GET `/v1/all`: Return all available Hanzo Flow component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are most often used when contributing to the Hanzo Flow codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
* MCP servers: The following endpoints are for managing Hanzo Flow MCP servers and MCP server connections.
They aren't typically called directly; instead, they are used to drive internal functionality in the Hanzo Flow frontend and when running flows that call MCP servers.
Hanzo Flow MCP servers support both streamable HTTP and SSE transport.
...
* Use the Hanzo Flow API to [run a flow](/api-flows-run).
* Use the Hanzo Flow API to [upload files](/api-files).
* Use the Hanzo Flow API to [get flow logs](/api-logs).
* Explore all endpoints in the [Hanzo Flow API specification](/api).
