...
Instead of manually exporting, sharing, and importing flow JSON files from the Hanzo Flow UI, the Flow DevOps toolkit offers terminal-based workflows for versioning, environment variables, testing, and deployment.

## Prerequisites

- [Install and start Hanzo Flow](/get-started-installation)
- Create a [Hanzo Flow API key](/api-keys-and-authentication)
- Install the Hanzo Flow `lfx` package
   ...
   2. Activate the virtual environment.
      ...
      ```
   3. Install the Hanzo Flow LFX package in the virtual environment:
      ```bash
      uv pip install lfx
      ```
   4. Run Flow DevOps Toolkit commands in the virtual environment that has LFX installed.

      Alternatively, you can run `uvx lfx` commands, or run LFX from the `src/lfx` directory in a cloned Hanzo Flow repo.
      For more information, see the [Hanzo Flow LFX README](https://github.com/hanzoai/flow/blob/main/src/lfx/README.md).
...
3. To initialize a project, run:
    ...
    ├── .github/
    │   └── workflows/
    │       ├── flow-push.yml       # CI workflow
    │       ├── flow-test.yml       # CI workflow
    │       └── flow-validate.yml   # CI workflow
    ...
    ✔ Project scaffolded. Next steps:
      ...
      2. export FLOW_LOCAL_API_KEY=<key>   (Settings -> API Keys)
...
4. Add your Hanzo Flow API key to your `.env` file, or export it within the terminal session.
    ...
    For example, to add a Hanzo Flow API key to a local Hanzo Flow server, set:

    ```text
    export FLOW_LOCAL_API_KEY=FLOW_API_KEY
    ```
...
7. In the Hanzo Flow UI, open the Simple Agent flow and change the flow.
...
9. To pull the reported changes from the Hanzo Flow server to your local project folder, run `lfx pull`.
10. To _push_ flow changes from flows stored locally in `demo-project/flows` to the Hanzo Flow server, run `lfx push`.

## Validate flows

The Flow DevOps SDK can validate that local flows are correctly formed before pushing to the Hanzo Flow with `lfx validate`.
...
2. Optionally, you can now share and serve the flow by keeping the flow JSON and `requirements.txt` in the same environment.
   To serve the flow without the Hanzo Flow UI, do the following:
   ...
   4. To set a Hanzo Flow API key, run:
   ```
   export FLOW_API_KEY=FLOW_API_KEY
   ```

   5. To serve the flow without the Hanzo Flow UI, pass the flow JSON path to the `lfx serve` command:
   ```
...
production:
  ...
  api_key_env: FLOW_PRODUCTION_API_KEY
```

Each entry contains a `url` for the Hanzo Flow base URL, and an `api_key_env` field.
...
`environments.yaml` is distinct from the Hanzo Flow or LFX `.env` file.
`environments.yaml` controls which remote Hanzo Flow instance you're deploying flows to, flow versioning, and environment variable _names_ for API keys.
The `.env` contains runtime values for the Hanzo Flow server, and might also contain _actual secret values_, so the `.env` should not be committed to version control.

Commands that call a Hanzo Flow server over HTTP, such as `lfx pull` or `lfx push`, use `--env ENVIRONMENT_NAME` to determine which Hanzo Flow instance to send the request to.
...
This command will send the request to the Hanzo Flow base URL at `http://127.0.0.1:7860` using a Hanzo Flow API key named `FLOW_LOCAL_API_KEY`.
