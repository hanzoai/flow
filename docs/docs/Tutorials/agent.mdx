...
This tutorial shows you how to connect a JavaScript application to a [Hanzo Flow agent](/agents).
...
In this example, the application sends a customer's email address to the Hanzo Flow agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    const FLOW_SERVER_ADDRESS = 'FLOW_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';
    const FLOW_API_KEY = 'FLOW_API_KEY';
    ...
    async function runAgentFlow(): Promise<void> {
        try {
            ...
            const client = new Hanzo FlowClient({
                baseUrl: FLOW_SERVER_ADDRESS,
                apiKey: FLOW_API_KEY
            });

            console.log(`Connecting to Hanzo Flow server at: ${FLOW_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your Hanzo Flow server is running and accessible at:', FLOW_SERVER_ADDRESS);
                }
    ...
    // Run the function
    console.log('Starting Hanzo Flow Agent...\n');
...
3.  Save and run the script to send the request and test the flow.
    ...
    ```
    Starting Hanzo Flow Agent...

    Connecting to Hanzo Flow server at: http://localhost:7860
