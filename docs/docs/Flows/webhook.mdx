...
To allow webhooks to run without authentication (not recommended; use only in trusted environments), in your Hanzo Flow `.env` file, set `LANGFLOW_WEBHOOK_AUTH_ENABLE=False`. When disabled, requests to the webhook endpoint are treated as being sent by the flow owner.
