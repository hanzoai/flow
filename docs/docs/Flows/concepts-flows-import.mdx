...
You can export flows to transfer them between Hanzo Flow instances, share them with others, or create backups.
...
* **Export with the Hanzo Flow API**: To export one flow, use the [`/flows/download`](/api-flows#export-flows) endpoint.
...
For more information, see [Hanzo Flow JSON file contents](#flow-json-file-contents).
...
If your key is stored in a Hanzo Flow global variable, **Save with my API keys** exports only the variable name.
:::

When you or another user import the flow to another Hanzo Flow instance, that instance must have Hanzo Flow global variables with the same names and valid values in order to run the flow successfully.
If any variables are missing or invalid, those variables must be created or edited after importing the flow.

## Import a flow

You can import Hanzo Flow JSON files from your local machine in the following ways:

* **Import to projects**: On the **Projects** page, click <Icon name="Upload" aria-hidden="true"/> **Upload a flow**, and then select the Hanzo Flow JSON file to import.
* **Import anywhere**: Drag and drop Hanzo Flow JSON files from your file explorer into your Hanzo Flow window to import a flow from any Hanzo Flow page.
* **Import with the Hanzo Flow API**: To import one Hanzo Flow JSON file, use the [`/flows/upload/`](/api-flows#import-flows) endpoint.
To import a zip archive of Hanzo Flow JSON files, use the [`/projects/upload`](/api-projects#import-a-project) endpoint.

### Run an imported flow

Once imported, your flow is ready to use.
If the flow contains any global variables, make sure your Hanzo Flow instance has global variables with the same names and valid values.
For more information, see [Save with my API keys](/concepts-flows-import#save-with-my-api-keys).

## Hanzo Flow JSON file contents

An exported flow is downloaded to your local machine as a JSON file named `FLOW_NAME.json`.

Hanzo Flow JSON files contain the following:
...
For example Hanzo Flow JSON files, you can examine any of the https://github.com/hanzoai/flow/tree/main/src/backend/base/flow/initial_setup/starter_projects[templates in the Hanzo Flow repository], or you can create a flow from a template in Hanzo Flow, export it, and then open the exported JSON file in a text editor.
