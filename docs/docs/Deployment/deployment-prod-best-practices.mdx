...
* **Resource usage**: Track CPU, memory, and disk usage of Hanzo Flow instances. Use Prometheus and Grafana for real-time metrics collection and monitoring in Kubernetes.
    ...
    To expose your Hanzo Flow server's Prometheus metrics, set `FLOW_PROMETHEUS_ENABLED=True` (the default is `false`).
    The default port for the Prometheus metrics is 9090.
    To change the port, set `FLOW_PROMETHEUS_PORT`.
