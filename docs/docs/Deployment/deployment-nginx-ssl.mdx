---
title: Deploy Hanzo Flow with Nginx and SSL
slug: /deployment-nginx-ssl
---

import Icon from "@site/src/components/icon";

Deploy Hanzo Flow on a Linux-based server using Nginx as a reverse proxy, Let's Encrypt for SSL certificates, and Certbot for automated certificate management.

This setup encrypts all communications between users and your Hanzo Flow server. SSL certificates ensure that sensitive data is protected from eavesdropping and tampering, and the automatic certificate management through Certbot eliminates the complexity of manual SSL configuration.
...
## Install Hanzo Flow on your server

To install Hanzo Flow on your server, do the following:
...
3. Install uv to manage Python packages.
    Hanzo Flow recommends uv for faster installation.
    ```bash
    pip install uv
    ```

4. Create a virtual environment for Hanzo Flow.
    ```bash
    uv venv flow-venv
    source flow-venv/bin/activate
    ```

5. Install Hanzo Flow using uv.
    ```bash
    uv pip install flow
    ```

6. Optionally, start Hanzo Flow.
    ```bash
    uv run flow run --host 127.0.0.1 --port 7860 &
    ```

    Test access at your `http://YOUR_PUBLIC_IP:7860`.
    Since Hanzo Flow is running on `localhost`, you should not be successful.
...
Nginx is a reverse proxy that receives external requests and forwards them to your Hanzo Flow server. In addition to SSL termination, Nginx includes features for load balancing and security. For more information, see the [Nginx documentation](https://nginx.org/en/docs/).
...
Replace **DOMAIN_NAME** with your actual domain name, such as `flow.example.com`.
    ```bash
...
4. Paste the following content to the configuration file you created at `/etc/nginx/sites-available/DOMAIN_NAME`.
    ...
    server {
        ...
        location / {
            ...
            # WebSocket support for Hanzo Flow
...
3. Start Hanzo Flow from the virtual environment.
    ...
    b. Start Hanzo Flow in the background.
    ```bash
    uv run flow run --host 127.0.0.1 --port 7860 &
    ```
...
- [Deploy Hanzo Flow on a remote server with Caddy](/deployment-caddyfile)
