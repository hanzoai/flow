...
This page describes the components that are available in the **Hanzo Datastore** bundle.

## Hanzo Datastore vector store

The **Hanzo Datastore** component reads and writes to a Hanzo Datastore vector store using an instance of `Hanzo Datastore` vector store.
...
For information about accepted values and functionality, see the [Hanzo Datastore documentation](https://docs.hanzo.ai/docs/datastore) or inspect [component code](/concepts-components#component-code).

| Name | Display Name | Info |
|------|--------------|------|
| host | hostname | Input parameter. The Hanzo Datastore server hostname. Required. Default: `localhost`. |
| port | port | Input parameter. The Hanzo Datastore server port. Required. Default: `8123`. |
| database | database | Input parameter. The Hanzo Datastore database name. Required. |
| table | Table name | Input parameter. The Hanzo Datastore table name. Required. |
| username | Username | Input parameter. Hanzo Datastore username for authentication. Required. |
| password | Password | Input parameter. Hanzo Datastore password for authentication. Required. |
...
| secure | Use HTTPS/TLS | Input parameter. If `true`, enables HTTPS/TLS for the Hanzo Datastore server and overrides inferred values for interface or port arguments. Default: `false`. |
