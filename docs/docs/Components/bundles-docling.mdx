...
The Docling dependency is required to use the Docling components in Hanzo Flow.
    ...
    * **Hanzo Flow Desktop**: Set `FLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for Hanzo Flow Desktop](/environment-variables#set-environment-variables-for-flow-desktop).
