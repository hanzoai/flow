...
<Icon name="Blocks" aria-hidden="true" /> [**Bundles**](/components-bundle-components) contain custom components that support specific third-party integrations with Hanzo Flow.

Hanzo Flow integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing and chunking documents.
...
The Docling dependency is required to use the Docling components in Hanzo Flow.

    * **Hanzo Flow Desktop**: Set `LANGFLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for Hanzo Flow Desktop](/environment-variables#set-environment-variables-for-flow-desktop).

    * **Hanzo Flow OSS**:
        * If you installed `flow` (`uv pip install flow`), Docling is included automatically through bundled extras, without Docling's optional chunking dependencies.
        * If you installed `flow-base` directly, install Docling with an extra, for example `uv pip install "flow-base[docling]"`.
        * To use the **Chunk DoclingDocument** component, install the chunking extra with `uv pip install "flow[docling-chunking]"` or `uv pip install "flow-base[docling-chunking]"`.
    ...
    * **Earlier versions**: Hanzo Flow versions earlier than 1.6 don't include the Docling dependency by default.
...
:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + Hanzo Flow: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
...
This component requires Docling's optional chunking dependencies. Install them with `uv pip install "flow[docling-chunking]"` or `uv pip install "flow-base[docling-chunking]"`.
