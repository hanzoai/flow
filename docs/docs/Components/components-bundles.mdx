...
Bundles contain custom components that support specific third-party integrations with Hanzo Flow.
You add them to your flows and configure them in the same way as Hanzo Flow's core components.
...
Many bundled components are developed by third-party contributors to the Hanzo Flow codebase.
...
To find documentation for a specific bundled component, browse the Hanzo Flow docs and your provider's documentation.
...
The Hanzo Flow documentation focuses on using bundles within flows.
For that reason, it focuses on the Hanzo Flow-specific configuration steps for bundled components.
...
:::tip
The Hanzo Flow documentation doesn't list all bundles or components in bundles.
For the most accurate and up-to-date list of bundles and components for your version of Hanzo Flow, check <Icon name="Blocks" aria-hidden="true" /> **Bundles** in the visual editor.

If you can't find a component that you used in an earlier version of Hanzo Flow, it may have been removed or marked as a [legacy component](#legacy-bundles).
:::

Hanzo Flow offers generic <Icon name="Component" aria-hidden="true" /> **Core components** in addition to third-party, provider-specific bundles.
...
The **Local DB** component reads and writes to a persistent, in-memory Chroma DB instance intended for use with Hanzo Flow.
It has separate modes for reads and writes, automatic collection management, and default persistence in your Hanzo Flow cache directory.
...
<!-- Not documented but in Hanzo Flow as of 1.5.11 -->
<!--
* AgentQL
* Confluence
* Firecrawl
