...
Variables in a **Prompt Template** component dynamically add fields to the **Prompt Template** component so that your flow can receive definitions for those values from other components, Hanzo Flow global variables, or fixed input.
...
5. Provide input for the variable fields:
   ...
   * Use Hanzo Flow global variables.
...
Hanzo Flow supports variable replacement with double brackets, but does not support the full Mustache engine.
