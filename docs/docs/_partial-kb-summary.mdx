A Hanzo Flow knowledge base is a local vector database that is stored in Hanzo Flow storage.
...
* **Local storage**: Hanzo Flow knowledge bases are exclusively local.
In contrast, only some vector store components support local databases.
* **Built-in embedding models**: Hanzo Flow knowledge bases include built-in support for several embedding models.
...
* **Basic similarity search**: When querying Hanzo Flow knowledge bases, only standard similarity search is supported.
...
* **Structured data**: Hanzo Flow knowledge bases only support structured data.
