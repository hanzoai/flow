...
:::info
Voice mode is not available in Hanzo Flow Desktop.
To use voice mode, [Install the Hanzo Flow OSS Python package](/get-started-installation#install-and-run-the-flow-oss-python-package).
:::

You can use Hanzo Flow's voice mode to interact with your flows verbally through a microphone and speakers.
...
* An [OpenAI](https://platform.openai.com/) account and an OpenAI API key because Hanzo Flow uses the OpenAI API to process voice input and generate responses.
...
5. Enter your OpenAI API key, and then click **Save**. Hanzo Flow saves the key as a [global variable](/configuration-global-variables).
...
Hanzo Flow saves this key as a global variable.
...
Hanzo Flow exposes two OpenAI Realtime API-compatible websocket endpoints for your flows.
...
The Hanzo Flow API's websockets endpoints require an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and they support an optional [ElevenLabs](https://elevenlabs.io) integration with an ElevenLabs API key.
...
This is the mode used in the Hanzo Flow **Playground**.
...
If omitted, Hanzo Flow uses the flow ID as the [session ID](/session-id).
