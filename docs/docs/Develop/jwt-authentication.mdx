...
No configuration is necessary, but if you want to explicitly set it in the Hanzo Flow `.env`, the default value is `LANGFLOW_ALGORITHM=HS256`.
...
To automatically generate a private and public key pair and store it in the Hanzo Flow [`LANGFLOW_CONFIG_DIR`](/logging), set `LANGFLOW_ALGORITHM="RS256"` in your Hanzo Flow `.env`.
...
To automatically generate a private and public key pair and store it in the Hanzo Flow [`LANGFLOW_CONFIG_DIR`](/logging), set `LANGFLOW_ALGORITHM="RS512"` in your Hanzo Flow `.env`.
...
services:
  flow:
    ...
    environment:
      - LANGFLOW_ALGORITHM=RS256
    volumes:
      - langflow_data:/app/flow  # Keys stored here
