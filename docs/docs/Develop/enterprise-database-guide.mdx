...
3. Configure Hanzo Flow with the `.env` or `docker-compose.yml` files.
    ...
    2. Add the connection string to the `.env` file:
        ...
        ```text
        LANGFLOW_DATABASE_URL="postgresql://flow:securepassword@postgres:5432/flow"
        ```
    ...
    services:
      flow:
        ...
        ports:
          - "7860:7860"
        environment:
          - LANGFLOW_DATABASE_URL=postgresql://flow:flow@postgres:5432/flow
      postgres:
...
Hanzo Flow, through [SQLAlchemy](https://docs.sqlalchemy.org/en/20/), supports reconnection attempts if `LANGFLOW_DATABASE_CONNECTION_RETRY=True`, ensures recovery after failover, and reduces disruption once the database is back online.
