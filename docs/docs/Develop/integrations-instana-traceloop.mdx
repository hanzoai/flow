...
description: Instrument Hanzo Flow with the Traceloop SDK, and export traces and metrics to Instana using OpenTelemetry.
---
...
This guide demonstrates how to integrate the Instana observability platform with your Hanzo Flow application using the Traceloop SDK so you can monitor and analyze LLM performance.
...
1. In the root folder of your Hanzo Flow application, edit your existing Hanzo Flow `.env` file or create a new one.
...
## Start Hanzo Flow with Traceloop environment variables

Launch your Hanzo Flow application with your `.env` file:
...
1. Run a flow in Hanzo Flow to generate traffic.
2. To view traces in Instana, open Instana and click **Applications**.
3. In **Services**, search for `Hanzo Flow`.
4. Click **Hanzo Flow** to view and analyze the associated calls.
