...
Hanzo Flow provides several components to help you store and retrieve vector data in your flows, including embedding models, vector stores, and knowledge bases.
...
Knowledge bases are stored local to your Hanzo Flow instance.
The default storage location depends on your operating system and installation method:

- **Hanzo Flow Desktop**:
    - **macOS**: `/Users/<username>/.flow/knowledge_bases`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.Hanzo FlowDesktop\knowledge_bases`
- **Hanzo Flow OSS**:
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/flow/knowledge_bases` (Python version can vary. Knowledge bases aren't shared between virtual environments.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/flow/knowledge_bases`

If you set the `FLOW_CONFIG_DIR` environment variable, the `knowledge_bases` subdirectory is created relative to that path.

To change the default `knowledge_bases` directory path, set the `FLOW_KNOWLEDGE_BASES_DIR` environment variable:

```bash
export FLOW_KNOWLEDGE_BASES_DIR="/path/to/parent/directory"
```
