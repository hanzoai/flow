...
Get to know Hanzoflow by building an OpenAI-powered chatbot application. After you've constructed a chatbot, add Retrieval Augmented Generation (RAG) to chat with your own data.
...
1. From the Hanzoflow dashboard, click **New Flow**, and then select **Blank Flow**. A blank workspace opens where you can build your flow.
...
1. Add your credentials to the OpenAI component. The fastest way to complete these fields is with Hanzoflow’s [Global Variables](/configuration-global-variables).
...
You have successfully created a chatbot application using OpenAI in the Hanzoflow Workspace.

## Add vector RAG to your application

You created a chatbot application with Hanzoflow, but let's try an experiment.
