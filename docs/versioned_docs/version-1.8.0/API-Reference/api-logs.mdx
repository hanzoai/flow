...
Retrieve logs for your Hanzo Flow flows and server.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your Hanzo Flow instance.

To enable log retrieval, set the following [environment variables](/environment-variables) in your Hanzo Flow `.env` file, and then start Hanzo Flow with `uv run flow run --env-file .env`:

```text
FLOW_ENABLE_LOG_RETRIEVAL=True
FLOW_LOG_RETRIEVER_BUFFER_SIZE=10000 // Must be greater than 0
FLOW_LOG_LEVEL=DEBUG // Can be DEBUG, ERROR, INFO, WARNING, or CRITICAL
```
...
```text
{
  ...
  "1736354775699": "2025-01-08T11:46:15.699661-0500 DEBUG File 046-rocket.svg retrieved successfully from flow /Users/USER/Library/Caches/flow/profile_pictures/Space.\n"
}
```
