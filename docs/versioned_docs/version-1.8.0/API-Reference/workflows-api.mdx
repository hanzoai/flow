...
The Workflow API provides programmatic access to execute Hanzo Flow workflows synchronously or asynchronously.
...
The Workflow API is part of the Hanzo Flow Developer v2 API and offers enhanced workflow execution capabilities compared to the v1 `/run` endpoint.
...
url = f"{FLOW_SERVER_URL}/api/v2/workflows"
headers = {
...
const url = `${FLOW_SERVER_URL}/api/v2/workflows`;
...
url = f"{FLOW_SERVER_URL}/api/v2/workflows/stop"
headers = {
    "Content-Type": "application/json",
    "x-api-key": FLOW_API_KEY
}
...
const url = `${FLOW_SERVER_URL}/api/v2/workflows/stop`;
...
To find the component ID in the Hanzo Flow UI, open your flow in Hanzo Flow, click the component, and then click **Controls**. The component ID is at the top of the **Controls** pane.
