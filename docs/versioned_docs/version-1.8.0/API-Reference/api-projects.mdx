...
Use the `/projects` endpoint to create, read, update, and delete [Hanzo Flow projects](/concepts-flows#projects).

## Read projects

Get a list of Hanzo Flow projects, including project IDs, names, and descriptions.
...
Import a project and its flows by uploading a Hanzo Flow project zip file:
...
curl -X POST \
  ...
  -H "x-api-key: $FLOW_API_KEY" \
  -F "file=@20241230_135006_flow_flows.zip;type=application/zip"
```