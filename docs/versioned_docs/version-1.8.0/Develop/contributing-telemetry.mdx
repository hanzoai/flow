...
Hanzo Flow uses anonymous telemetry to collect statistics about feature usage and performance.
The Hanzo Flow team uses this data to identify popular features and areas that need improvement based on actual usage patterns.
This helps prioritize development efforts on the most impactful changes and popular features.

## Privacy

The Hanzo Flow team respects your privacy, and the team is committed to protecting your data.

Hanzo Flow telemetry doesn't collect any personal information or sensitive data.
All telemetry data is anonymized and used solely for improving Hanzo Flow.

## Opt out of telemetry

To opt out of telemetry, set `DO_NOT_TRACK=True` in your [Hanzo Flow environment variables](/environment-variables) before starting Hanzo Flow. This disables telemetry data collection.

## Data that Hanzo Flow collects

Hanzo Flow telemetry collects data on flow runs, your environment, and component usage.
...
- **Version**: The specific version of Hanzo Flow used, which helps in tracking feature adoption and compatibility.
...
- **Arch**: Architecture of the system, such as x86 or ARM, which helps prioritize hardware optimization and testing in the Hanzo Flow codebase.
...
- **BackendOnly**: Boolean indicating whether Hanzo Flow is running in backend-only mode, useful for understanding deployment configurations.
- **Desktop**: Indicates whether Hanzo Flow is running in desktop mode (Hanzo Flow Desktop), helping to understand usage patterns across different deployment types.

### Email

This telemetry event is sent to track registered email addresses for Hanzo Flow Desktop. The event is triggered in two cases:

* Every time a new email address is registered through the POST `/api/v2/registration/` endpoint.
* Each time you start Hanzo Flow Desktop _after_ an email address is registered.

   The first time you start Hanzo Flow Desktop and register your email address, the event is reported by the call to the POST `/api/v2/registration/` endpoint.

This telemetry event includes the following information:

- **Email**: The registered email address, which helps track user registrations and facilitate an understanding of the Hanzo Flow Desktop user base.
- **ClientType**: Indicates the client type, which can be "desktop" or "oss".

If telemetry is disabled with the `DO_NOT_TRACK` environment variable in Hanzo Flow Desktop, you are still prompted to enter your email address, but the email address is stored in your local Hanzo Flow database only.
...
This telemetry event is sent when an unhandled exception is captured by Hanzo Flow's lifecycle or global exception handler.
