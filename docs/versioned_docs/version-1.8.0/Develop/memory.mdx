...
* **Config directory**: Set `LANGFLOW_SAVE_DB_IN_CONFIG_DIR=True` to store the database in your Hanzo Flow config directory as set in [`LANGFLOW_CONFIG_DIR`](/logging).
...
* **No database**: To disable all database operations and run a no-op session, set `LANGFLOW_USE_NOOP_DATABASE=True` in your [Hanzo Flow environment variables](/environment-variables).
...
• **Flow**: Contains flow definitions, including nodes, edges, and components, stored as JSON or database records. For more information, see [Build flows](/concepts-flows).
    ...
    :::tip
    To automatically remove API keys and tokens from flow data before saving a flow to the database, set `LANGFLOW_REMOVE_API_KEYS=True` in your [Hanzo Flow environment variables](/environment-variables).
...
To replace the default Hanzo Flow SQLite database with another database, set the `LANGFLOW_DATABASE_URL` environment variable to your database URL, and then start Hanzo Flow with your `.env` file.
...
* `LANGFLOW_DATABASE_CONNECTION_RETRY`: Whether to retry lost connections to your Hanzo Flow database. If `true`, Hanzo Flow tries to connect to the database again if the connection fails. Default: `false`.
...
* `LANGFLOW_MIGRATION_LOCK_NAMESPACE`: Optional namespace identifier for PostgreSQL advisory lock during migrations. If not provided, a hash of the database URL will be used. Useful when multiple Hanzo Flow instances share the same database and need coordinated migration locking
...
|----------|------|---------|-------------|
| `LANGFLOW_CACHE_TYPE` | String | `async` | Set the cache type for Hanzo Flow's internal caching system. Possible values: `async`, `redis`, `memory`, `disk`. If you set the type to `redis`, then you must also set the `LANGFLOW_REDIS_*` environment variables. |
| `LANGFLOW_LANGCHAIN_CACHE` | String | `InMemoryCache` | Set the cache storage type for the LangChain caching system (a Hanzo Flow dependency), either `InMemoryCache` or `SQLiteCache`. |
