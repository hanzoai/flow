...
To enable Arize tracing, set the required Arize environment variables in your Hanzo Flow deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating Hanzo Flow and Arize are also available in the Arize documentation:

* [Hanzo Flow tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/flow/flow-tracing)
* [Hanzo Flow tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/flow/flow-tracing)
:::
...
2. In the root of your Hanzo Flow application, edit your existing Hanzo Flow `.env` file or create a new one.
...
4. Start your Hanzo Flow application with your `.env` file:
...
1. In Hanzo Flow, run a flow that has an LLM-driven component, such as an **Agent** component or any language model component.
...
3. To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each Hanzo Flow execution generates two traces in Arize:

    * The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    * The `UUID` trace is the trace of the Hanzo Flow components.
