...
Hanzo Flow integrates with [Google BigQuery](https://cloud.google.com/bigquery) through the **BigQuery** component in the [**Google** bundle](/bundles-google), allowing you to execute SQL queries and retrieve data from your BigQuery datasets.
...
* A [running Hanzo Flow server](/get-started-installation)
...
This role allows Hanzo Flow to access BigQuery resources with the service account.
...
Now that you have a service account and a JSON private key, you need to configure the credentials in the Hanzo Flow **BigQuery** component.

#### Configure credentials in the Hanzo Flow component

With your service account configured and your credentials JSON file created, follow these steps to authenticate the Hanzo Flow application.

1. Create a new flow in Hanzo Flow.
...
2. Open the **Playground**, and then submit a valid SQL query.
    ...
    FROM `big-query-flow-project.the_oscar_award.oscar_winners`
    WHERE winner = TRUE
    LIMIT 10
    ```
...
Hanzo Flow includes multiple components that support Google Search, such as the following:
...
The **Google OAuth Token** component was deprecated in Hanzo Flow 1.4.0.
