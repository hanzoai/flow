...
The Docling dependency is required to use the Docling components in Hanzo Flow.
    ...
    * **Hanzo Flow version 1.6 and later**: The Docling dependency is included by default for all operating systems except macOS Intel (x86_64).
    ...
    * **Earlier versions**: Hanzo Flow versions earlier than 1.6 don't include the Docling dependency.
    For Hanzo Flow OSS, install the Docling extra with `uv pip install 'flow[docling]'`.
    For Hanzo Flow Desktop, add the Docling dependency to Hanzo Flow Desktop's `requirements.txt`.
