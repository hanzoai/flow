...
1. Install the Agentics package in Hanzo Flow's virtual environment:

   ```bash
   uv pip install agentics-py==0.3.1
   ```

2. Restart Hanzo Flow so the Agentics components are available:
...
- **Errors:** If Agentics is not found, run `uv pip install agentics-py==0.3.1` and restart Hanzo Flow. For API/key errors, set the provider’s API key as a global variable or env var. For DataFrame errors, ensure input is a list of dicts or use a DataFrame component output.
