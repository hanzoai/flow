...
This tutorial demonstrates how you can use Hanzo Flow to create a chatbot application that uses Retrieval Augmented Generation (RAG) to embed your data as vectors in a vector database, and then chat with the data.
...
* Install the [Hanzo Flow JavaScript client](/typescript-client)
* Be familiar with vector search concepts and applications, such as vector databases and RAG

## Create a vector RAG flow

1. In Hanzo Flow, click **New Flow**, and then select the **Vector Store RAG** template.
...
In situations where many users load data or you need to load data programmatically, use the Hanzo Flow API option.
...
2. Select the local file you want to upload, and then click **Open**.
    The file is loaded to your Hanzo Flow server.
...
<TabItem value="API" label="Hanzo Flow API">

To load data programmatically, use the `/v2/files/` and `/v1/run/$FLOW_ID` endpoints. The first endpoint loads a file to your Hanzo Flow server, and then returns an uploaded file path. The second endpoint runs the **Load Data Flow**, referencing the uploaded file path, to chunk, embed, and load the data into the vector store.
...
const headers = { 'x-api-key': 'FLOW_API_KEY' };

// 2. Upload the file to Hanzo Flow
const uploadRes = await fetch('FLOW_SERVER_ADDRESS/api/v2/files/', {
  method: 'POST',
  headers,
  body: data
});
...
// 3. Call the Hanzo Flow run endpoint with the uploaded file path
const payload = {
...
const runRes = await fetch('FLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', 'x-api-key': 'FLOW_API_KEY' },
  body: JSON.stringify(payload)
});
const flowData = await runRes.json();
// Output only the message
console.log(flowData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text);
```
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    const { Hanzo FlowClient } = require('@datastax/flow-client');

    # pragma: allowlist nextline secret
    const API_KEY = 'FLOW_API_KEY';
    const SERVER = 'FLOW_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';

    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });

    // Initialize the Hanzo Flow client
    const client = new Hanzo FlowClient({
        baseUrl: SERVER,
        apiKey: API_KEY
    });
    ...
    function chat() {
        console.log('🤖 Hanzo Flow RAG Chatbot (type "quit" to exit)\n');
    ...
    :::tip
    The [Hanzo Flow TypeScript client](/typescript-client) has a `chatOutputText()` convenience method that simplifies working with Hanzo Flow's complex JSON response structure.
