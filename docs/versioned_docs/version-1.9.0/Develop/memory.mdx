...
Hanzo Flow provides flexible memory management options for storage and retrieval of data relevant to your flows and your Hanzo Flow server.
This includes essential Hanzo Flow database tables, file management, and caching, as well as chat memory.

## Storage options and paths

Hanzo Flow supports both local memory and external memory options.

Hanzo Flow's default storage option is a [SQLite](https://www.sqlite.org/) database.
The default storage path depends on your operating system and installation method:

- **Hanzo Flow Desktop**:
    - **macOS**: `/Users/<username>/.flow/data/database.db`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.Hanzo FlowDesktop\data\database.db`
- **Hanzo Flow OSS**
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/flow/flow.db` (Python version can vary. Database isn't shared between virtual environments because it is tied to the venv path.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/flow/flow.db`

Hanzo Flow offers a few alternatives to the default database path:

* **Config directory**: Set `FLOW_SAVE_DB_IN_CONFIG_DIR=True` to store the database in your Hanzo Flow config directory as set in [`FLOW_CONFIG_DIR`](/logging).

* **External PostgreSQL database**: You can use an external PostgreSQL database for all of your Hanzo Flow storage.
For more information, see [Configure external memory](#configure-external-memory)

    External storage can be useful if you want to preserve the data after uninstalling Hanzo Flow or to share the same database between multiple virtual environments.

* **Separate chat memory**: You can selectively use external storage for chat memory only, separate from other Hanzo Flow storage.
For more information, see [Store chat memory](#store-chat-memory).

* **No database**: To disable all database operations and run a no-op session, set `FLOW_USE_NOOP_DATABASE=True` in your [Hanzo Flow environment variables](/environment-variables).
This is useful for testing when you don't want to persist any data.

## Hanzo Flow database tables

The following tables are stored in `flow.db`:

• **ApiKey**: Manages Hanzo Flow API authentication keys. Component API keys are stored in the **Variables** table. For more information, see [API keys and authentication](/api-keys-and-authentication).

• **File**: Stores metadata for files uploaded to Hanzo Flow's file management system, including file names, paths, sizes, and storage providers. For more information, see [Manage files](/concepts-file-management).
...
• **Flow**: Contains flow definitions, including nodes, edges, and components, stored as JSON or database records. For more information, see [Build flows](/concepts-flows).
    ...
    :::tip
    To automatically remove API keys and tokens from flow data before saving a flow to the database, set `FLOW_REMOVE_API_KEYS=True` in your [Hanzo Flow environment variables](/environment-variables).
...
For more information, see the database models in the [source code](https://github.com/hanzoai/flow/tree/main/src/backend/base/flow/services/database/models).

## Configure external memory {#configure-external-memory}

To replace the default Hanzo Flow SQLite database with another database, set the `FLOW_DATABASE_URL` environment variable to your database URL, and then start Hanzo Flow with your `.env` file.
...
* `FLOW_DATABASE_CONNECTION_RETRY`: Whether to retry lost connections to your Hanzo Flow database. If `true`, Hanzo Flow tries to connect to the database again if the connection fails. Default: `false`.

* `FLOW_DB_CONNECT_TIMEOUT`: The number of seconds to wait before giving up on a lock to be released or establishing a connection to the database. This may be separate from the `pool_timeout` in `FLOW_DB_CONNECTION_SETTINGS`. Default: 30.

* `FLOW_MIGRATION_LOCK_NAMESPACE`: Optional namespace identifier for PostgreSQL advisory lock during migrations. If not provided, a hash of the database URL will be used. Useful when multiple Hanzo Flow instances share the same database and need coordinated migration locking

* `FLOW_DB_CONNECTION_SETTINGS`: A JSON dictionary containing the following database connection pool settings:
    ...
    ```text
    FLOW_DB_CONNECTION_SETTINGS='{"pool_size": 20, "max_overflow": 30, "pool_timeout": 30, "pool_pre_ping": true, "pool_recycle": 1800, "echo": false}'
    ```

    Don't use the deprecated environment variables `FLOW_DB_POOL_SIZE` or `FLOW_DB_MAX_OVERFLOW`.
    Instead, use `pool_size` and `max_overflow` in `FLOW_DB_CONNECTION_SETTINGS`.

* `FLOW_MIGRATION_LOCK_NAMESPACE`: Optional namespace for PostgreSQL advisory locks used during database migrations. This is useful when running multiple Hanzo Flow instances that share the same PostgreSQL database. Each instance should use a unique namespace to avoid conflicts. If not set, Hanzo Flow uses a default namespace. This setting only applies when using PostgreSQL as your database backend.

## Configure cache memory

The default Hanzo Flow caching behavior is an asynchronous, in-memory cache:
...
Hanzo Flow officially supports only the default asynchronous, in-memory cache, which is suitable for most use cases.
...
|----------|------|---------|-------------|
| `FLOW_CACHE_TYPE` | String | `async` | Set the cache type for Hanzo Flow's internal caching system. Possible values: `async`, `redis`, `memory`, `disk`. If you set the type to `redis`, then you must also set the `FLOW_REDIS_*` environment variables. |
| `FLOW_LANGCHAIN_CACHE` | String | `InMemoryCache` | Set the cache storage type for the LangChain caching system (a Hanzo Flow dependency), either `InMemoryCache` or `SQLiteCache`. |
| `FLOW_REDIS_HOST` | String | `localhost` | Redis server hostname if `FLOW_CACHE_TYPE=redis`. |
| `FLOW_REDIS_PORT` | Integer | `6379` | Redis server port if `FLOW_CACHE_TYPE=redis`. |
| `FLOW_REDIS_DB` | Integer | `0` | Redis database number if `FLOW_CACHE_TYPE=redis`. |
| `FLOW_REDIS_CACHE_EXPIRE` | Integer | `3600` | Cache expiration time in seconds if `FLOW_CACHE_TYPE=redis`. |
| `FLOW_REDIS_PASSWORD` | String | Not set | Optional password for Redis authentication if `FLOW_CACHE_TYPE=redis`. |

## Store chat memory

Chat-based flows with a **Chat Input** or **Chat Output** component produce chat history that is stored in the Hanzo Flow `messages` table.
...
All messages are stored in [Hanzo Flow storage](#storage-options-and-paths), and the component provides minimal memory configuration options, such as the number of messages to retrieve.
...
* **Message History component**: By default, this component stores and retrieves memories from Hanzo Flow storage, unless you attach a third-party chat memory component. It provides a few more options for sorting and filtering memories, although most of these options are also built-in to the **Agent** component as configurable or fixed parameters.
...
Typically, this is necessary only if you have specific storage needs that aren't met by Hanzo Flow storage.
For example, if you want to manage chat memory data by directly working with the database, or if you want to use a different database than the default Hanzo Flow storage.

For more information and examples, see [**Message History** component](/message-history) and [Agent memory](/agents#agent-memory).

## See also

* [Hanzo Flow file management](/concepts-file-management)
* [Hanzo Flow logs](/logging)
