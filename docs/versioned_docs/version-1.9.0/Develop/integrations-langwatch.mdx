...
To integrate with Hanzo Flow, add your LangWatch API key as a Hanzo Flow environment variable:

1. Get a LangWatch API key from your LangWatch account.

2. Add the key to your Hanzo Flow `.env` file:
...
3. Restart Hanzo Flow with your `.env` file, if you modified the Hanzo Flow `.env`:
