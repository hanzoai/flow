...
Hanzo Flow's [**Agent** component](/components-agents) is critical for building agent flows.
...
The following steps explain how to create an agent flow in Hanzo Flow from a blank flow.
For a prebuilt example, use the **Simple Agent** template or the [Hanzo Flow quickstart](/get-started-quickstart).
...
To continue building on this tutorial, try connecting other tool components or [use Hanzo Flow as an MCP client](/mcp-client) to support more complex and specialized tasks.
...
An **Agent** component can use any Hanzo Flow component as a tool, including other agents and MCP servers.
...
Hanzo Flow agents have built-in chat memory that is enabled by default.
...
By default, the **Agent** component uses your Hanzo Flow installation's storage, and it retrieves a limited number of chat messages, which you can configure with the **Number of Chat History Messages** parameter.
