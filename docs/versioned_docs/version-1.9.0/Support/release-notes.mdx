---
title: Hanzo Flow release notes
slug: /release-notes
---
...
This page summarizes significant changes to Hanzo Flow in each release.
For all changes, see the [Changelog](https://github.com/hanzoai/flow/releases/latest).

Due to strict SemVer requirements, Hanzo Flow Desktop can have different patch versions than the core Hanzo Flow OSS Python package, but the major and minor versions are aligned.

## Prepare to upgrade

:::warning
Whenever possible, the Hanzo Flow team recommends installing new Hanzo Flow versions in a new virtual environment or VM before upgrading your primary installation.
...
To avoid the impact of potential breaking changes and test new versions, the Hanzo Flow team recommends the following upgrade process:

1. Recommended: [Export your projects](/api-projects#export-a-project) to create backups of your flows:

    ```bash
    curl -X GET \
    "$FLOW_SERVER_URL/api/v1/projects/download/$PROJECT_ID" \
...
2. Install the new version:
   ...
   * **Hanzo Flow OSS Python package**: Install the new version in a new virtual environment. For instructions, see [Install and run the Hanzo Flow OSS Python package](/get-started-installation#install-and-run-the-flow-oss-python-package).
   * **Hanzo Flow Docker image**: Run the new image in a separate container.
   * **Hanzo Flow Desktop**: To upgrade in place, open Hanzo Flow Desktop, and then click **Upgrade Available** in the Hanzo Flow header. If you want to isolate the new version, you must install Hanzo Flow Desktop on a separate physical or virtual machine, and then [import your flows](/concepts-flows-import) to the new installation.
...
For all changes, see the [Changelog](https://github.com/hanzoai/flow/releases).

### New features and enhancements

- Hanzo Flow MCP client for Bob (IBM) and Claude Code

    The **Settings** page now includes <Icon name="Terminal" aria-hidden="true"/> **Hanzo Flow MCP Client**, with instructions to connect **Bob (IBM)** and **Claude Code** to your Hanzo Flow instance.
    For more information, see [Hanzo Flow MCP client](../Agents/flow-mcp-client.mdx).
...
- MCP Tools: register servers first, and then add from the MCP sidebar
    ...
    **MCP Tools** is no longer added from the **Agents** component list. Register each MCP server under **Settings** > **MCP Servers*, or from the flow editor <McpIcon /> **MCP** sidebar.
    ...
    For more information, see [Use Hanzo Flow as an MCP client](/mcp-client#use-the-mcp-tools-component).

- Flow DevOps Toolkit SDK

    Use the Flow DevOps Toolkit SDK to manage local and remote flows with Git-style terminal commands.
    For more information, see the [Hanzo Flow Flow DevOps Toolkit SDK](../API-Reference/flow-devops-sdk.mdx).
...
- Renamed `Data` and `DataFrame` types
    ...
    For more information, see [Hanzo Flow data types](/data-types).
...
- Build components with **Hanzo Flow Assistant**

    **Hanzo Flow Assistant** is a graph-aware co-pilot that can create components from natural language prompts, directly within the Hanzo Flow workspace.
    For more information, see [Hanzo Flow Assistant](../Flows/flow-assistant.mdx).
...
- Vulnerability reporting now uses HackerOne
    ...
    For the full policy and what to include in a report, see the [Hanzo Flow Security Policy](https://github.com/hanzoai/flow/blob/main/SECURITY.md).

- Disable custom components

    Set `FLOW_ALLOW_CUSTOM_COMPONENTS=false` to disable custom components and in-editor editing of component code.
...
- Deploy flows to watsonx Orchestrate (beta)
    ...
    In Hanzo Flow 1.9.1, Hanzo Flow flows can now be published to IBM watsonx Orchestrate as tools that a watsonx Orchestrate agent can call.
    This workflow packages a selected flow version for use in IBM watsonx Orchestrate.
    For more information, see [Deploy Hanzo Flow on watsonx Orchestrate](../Deployment/deployment-wxo.mdx).

    As of Hanzo Flow 1.9.2, this feature is behind a feature flag. To enable it, set `FLOW_FEATURE_WXO_DEPLOYMENTS=true` before starting Hanzo Flow.
...
-   `flow-base` dependency structure refactored

    The `flow-base` package now uses granular optional dependency groups. As a result, many dependencies that were previously included in the `flow-base` installation were moved to optional extras.

    If you installed Hanzo Flow with `uv pip install flow`, this isn't a breaking change. Installing `flow` automatically installs `flow-base[complete]`, which includes all optional dependencies and maintains the same functionality as before.

    However, if you installed Hanzo Flow with `uv pip install flow-base` without specifying extra dependencies, this _is_ a breaking change.
    Some dependencies that were previously included by default are now available only through optional extras.
    Therefore, installing `flow-base` directly only installs the [core base dependencies](https://github.com/hanzoai/flow/blob/main/src/backend/base/pyproject.toml).

    If you installed `flow-base`, there are two ways to resolve dependency errors that result from this breaking change:

    * If you need the full set of dependencies, you must install `flow-base` with the `complete` extra:

        ```bash
        uv pip install "flow-base[complete]"
        ```

    * If you need specific dependencies, you must install `flow-base` with those optional dependency groups. For example:

       ```bash
       uv pip install "flow-base[postgresql,openai,chroma]"
       ```

    For more information about available optional dependency groups, see [Install optional dependency groups for `flow-base`](/install-custom-dependencies#install-optional-dependency-groups-for-flow-base).

-   Docker image does not include `uv` or `uvx`

    :::tip
    Starting with Hanzo Flow 1.8.1, the official Docker images include `uv` and `uvx` again.
    If you're using Hanzo Flow 1.8.0, follow the steps in this release note to add `uv` and `uvx` in a derived image.
    :::

    In Hanzo Flow 1.8.0, the Hanzo Flow Docker image does not include `uv` or `uvx` in the runtime image.
    This means that MCP server configurations, including the default Hanzo Flow MCP server, that rely on commands like `uvx mcp-proxy` will fail inside the container with a `command not found` error.

    If you use MCP from within a Hanzo Flow 1.8.0 Docker image, you must install `uv` in an image derived from the official `flowai/flow` image.

    To install `uv` and `uvx` in a derived image based on the official `flowai/flow` image, do the following:

    1. Create a `Dockerfile` in your project.
    2. Use the official Hanzo Flow image as the base and install `uv` into the existing virtual environment:
        ```dockerfile
        FROM flowai/flow:1.8.0
    ...
    For more information, see [Upgrade the Hanzo Flow Docker image](/deployment-docker#upgrade-the-flow-docker-image).
...
- Docling installation behavior differs between Desktop and OSS
    ...
    In Hanzo Flow 1.8.0, Docling behavior depends on how Hanzo Flow is installed.

    For Hanzo Flow Desktop installations, you need to set `FLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for Hanzo Flow Desktop](/environment-variables#set-environment-variables-for-flow-desktop).

    For Hanzo Flow OSS installations with `uv pip install flow`, Docling is included with the Hanzo Flow package.

    For Hanzo Flow OSS installations with `uv pip install flow-base`, install Docling explicitly as an extra with `uv pip install "flow-base[docling]"`.
...
- Developer API: `/workflow` synchronous endpoints (Beta)
    ...
    The Developer API is part of a larger effort to improve Hanzo Flow's APIs with enhanced capabilities and better developer experience.
...
- Traces and trace view
    ...
    Hanzo Flow now records execution traces for flows and components.
...
- More configuration options for JWT-based session authentication
    ...
    Hanzo Flow 1.8 offers additional configuration options for JWT algorithms, including support for RS256/RS512 algorithms, configurable keys, and token lifetimes. For more information, see [JWT authentication](/jwt-authentication).
...
- Docker AMD vs ARM image sizes
    ...
    Hanzo Flow 1.8.0 addresses the AMD vs ARM Docker image size gap.
...
- New [**Openlayer** observability integration](/integrations-openlayer)
    ...
    Configures Hanzo Flow to send tracing data to Openlayer for analysis, monitoring, and evaluation of your flow executions.
...
All Hanzo Flow versions upgrading to 1.7.0 are affected.

Don't upgrade to Hanzo Flow version 1.7.0.
...
1. Revert Hanzo Flow to version 1.6.9:

   ```bash
   uv pip install flow==1.6.9
   ```
...
- Support for streamable HTTP transport for MCP clients and servers
    ...
    Hanzo Flow now supports [streamable HTTP transport](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) for both MCP clients and servers.
    When using Hanzo Flow as an [MCP client](/mcp-client), you can connect to MCP servers with streamable HTTP transport.
    When using Hanzo Flow as an [MCP server](/mcp-server), clients can connect using streamable HTTP transport.
    SSE transport is still supported as a fallback for backwards compatibility.

- Webhook authentication

    Added the `FLOW_WEBHOOK_AUTH_ENABLE` environment variable for authenticating requests to the [`/webhook` endpoint](/api-flows-run#webhook-run-flow). When `FLOW_WEBHOOK_AUTH_ENABLE=TRUE`, webhook endpoints require API key authentication and validate that the authenticated user owns the flow being executed. When `FALSE`, no Hanzo Flow API key is required and all requests to the webhook endpoint are treated as being sent by the flow owner. For more information, see [Trigger flows with webhooks](/webhook).

- Configurable API key validation

    Added the `FLOW_API_KEY_SOURCE` environment variable to control how Hanzo Flow validates API keys. When set to `db`, Hanzo Flow validates API keys against keys stored in the database. When set to `env`, Hanzo Flow validates API keys against the `FLOW_API_KEY` environment variable. For more information, see [API keys and authentication](/api-keys-and-authentication#flow-api-key-source).

- SSRF protection

    Added SSRF (Server-Side Request Forgery) protection to the [**API Request** component](/api-request). HTTP redirects are disabled by default to prevent SSRF bypass attacks. To enable SSRF protection, set `FLOW_SSRF_PROTECTION_ENABLED=TRUE`. Configure allowed hosts with `FLOW_SSRF_ALLOWED_HOSTS`. Flows that relied on automatic redirects will need to enable it manually.

- Email registration in Hanzo Flow Desktop

    Hanzo Flow Desktop now includes an email registration screen at startup. The registered email address helps track user registrations and understand the user base. For more information, see [Telemetry](/contributing-telemetry#email).
...
Hanzo Flow versions 1.6.0 through 1.6.3 have a critical bug where environment variables from `.env` files aren't read.
...
If your `.env` file includes `AUTO_LOGIN=false`, upgrading to the impacted versions causes Hanzo Flow to fall back to default settings, potentially giving all users superuser access immediately upon upgrade.
Additionally, database credentials, API keys, and other sensitive configurations can't be loaded from `.env` files.

_Don't_ upgrade to any Hanzo Flow version from 1.6.0 through 1.6.3 if you use `.env` files for configuration.
...
:::warning
Windows users of Hanzo Flow Desktop should **not** use the in-app update feature to upgrade to Hanzo Flow version 1.6.0.
Only Windows Desktop users upgrading to Hanzo Flow version 1.6.0 are affected.
:::

The **Update** button in Hanzo Flow Desktop will not work for Windows users, and may result in data loss.

Instead, download a fresh installation from the [Hanzo Flow website](https://flow.org) when a new version is available.

Follow the instructions below to minimize the risk of losing flows.

These instructions assume the default path for the Hanzo Flow database of `C:\Users\YOUR_USERNAME\AppData\Roaming\com.flow\data\database.db` or `C:\Users\%YOUR_USERNAME%\AppData\Roaming\com.flow\data\database.db`. Replace `YOUR_USERNAME` with your Windows username.

1. Ensure you have Administrator privileges.
2. Ensure you have enough disk space for a second installation of Hanzo Flow.
3. Close Hanzo Flow, and ensure no Hanzo Flow process is running in Task Manager.
4. The Hanzo Flow 1.6.0 installer automatically performs a database backup in a later step, but an additional manual backup provides additional redundancy and is recommended.
To manually back up your Hanzo Flow database file, do the following:
   1. In Windows Explorer, navigate to `C:\Users\YOUR_USERNAME\AppData\Roaming\com.flow\data\`. Replace `YOUR_USERNAME` with your Windows username.
   2. Copy the `database.db` at this location, and paste it to a safe location.
5. Download the Windows installer from the [Hanzo Flow website](https://flow.org).
...
The Hanzo Flow 1.6.0 installer automatically performs a database backup.
These steps install Hanzo Flow from scratch, and result in two Hanzo Flow installations: the previously installed version, and version 1.6.0.
This is expected behavior.
8. Start version 1.6.0 of Hanzo Flow, and confirm your flows behave as expected.
9. **If flows are missing**, restore your flows from your manual backup by doing the following:
    ...
    4. Start Hanzo Flow, and confirm your flows behave as expected.
    5. After confirmation, uninstall the previous version of Hanzo Flow, and keep version 1.6.0.

### Breaking changes

- Authentication enforced for Hanzo Flow API requests by default

    In Hanzo Flow version 1.6, `FLOW_AUTO_LOGIN=True` and `FLOW_SKIP_AUTH_AUTO_LOGIN=False` by default.
    This enforces authentication for Hanzo Flow API requests while still automatically authenticating all users as superusers in the visual editor.
    ...
    **This is a breaking change from 1.5 where both of these environment variables were `true` by default, bypassing all authentication.**
    ...
    For more information, see the documentation for [`FLOW_AUTO_LOGIN`](/api-keys-and-authentication#flow-auto-login).

### New features and enhancements

- OpenAI Responses API compatibility

  Hanzo Flow now includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses) at `POST /api/v1/responses`.
...
- Advanced document parsing with built-in Docling support
  ...
  To make it easier to use the [**Docling** components](/bundles-docling) and the **Read File** component's new advanced parsing feature, the Docling dependency is now included with Hanzo Flow for all operating systems except macOS Intel (x86_64).
...
- Hanzo Flow API requests can require authentication

    To enhance security and ensure proper authentication for automatic login features, Hanzo Flow API endpoints now require authentication with a Hanzo Flow API key, even when `FLOW_AUTO_LOGIN=True`.
    This change will be enforced in a future release.
    For temporary backwards compatibility, this release adds the `FLOW_SKIP_AUTH_AUTO_LOGIN` environment variable.
    The default value is `true`, which disables API authentication enforcement.
    To enforce API authentication, set `FLOW_SKIP_AUTH_AUTO_LOGIN=False`.
    For more information, see the documentation for [`FLOW_AUTO_LOGIN`](/api-keys-and-authentication#flow-auto-login).
...
- MCP server one-click installation
    ...
    On your Hanzo Flow project's **MCP server** page, click **Auto install** to install your Hanzo Flow MCP server to MCP clients with just one click.
    The option to install with a JSON configuration file is available for macOS, Windows, and WSL.
    For more information, see [Use Hanzo Flow as an MCP server](/mcp-server).

- MCP server management

    You can now add, remove, and edit your MCP servers in the **MCP Tools** components and through your Hanzo Flow **Settings** page.
    For more information, see [Use Hanzo Flow as an MCP client](/mcp-client).
...
- Stability improvements
    ...
    See an issue? [Raise it on GitHub](https://github.com/hanzoai/flow/issues).
...
- Added an enhanced **Breaking Changes** feature to help update components without breaking flows after updating Hanzo Flow.
...
- Introduced MCP server functionality to serve Hanzo Flow tools to MCP-compatible clients.
...
See the [Changelog](https://github.com/hanzoai/flow/releases).