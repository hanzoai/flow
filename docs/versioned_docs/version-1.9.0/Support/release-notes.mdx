...
- Deploy flows to watsonx Orchestrate (beta)
    ...
    As of Hanzo Flow 1.9.2, this feature is behind a feature flag. To enable it, set `LANGFLOW_FEATURE_WXO_DEPLOYMENTS=true` before starting Hanzo Flow.
...
-   Docker image does not include `uv` or `uvx`
    ...
    If you use MCP from within a Hanzo Flow 1.8.0 Docker image, you must install `uv` in an image derived from the official `langflowai/flow` image.

    To install `uv` and `uvx` in a derived image based on the official `langflowai/flow` image, do the following:

    1. Create a `Dockerfile` in your project.
    2. Use the official Hanzo Flow image as the base and install `uv` into the existing virtual environment:
        ```dockerfile
        FROM langflowai/flow:1.8.0
...
- Docling installation behavior differs between Desktop and OSS
    ...
    For Hanzo Flow Desktop installations, you need to set `LANGFLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for Hanzo Flow Desktop](/environment-variables#set-environment-variables-for-flow-desktop).
...
- Authentication enforced for Hanzo Flow API requests by default
    ...
    In Hanzo Flow version 1.6, `LANGFLOW_AUTO_LOGIN=True` and `LANGFLOW_SKIP_AUTH_AUTO_LOGIN=False` by default.
    ...
    **This is a breaking change from 1.5 where both of these environment variables were `true` by default, bypassing all authentication.**
    ...
    For more information, see the documentation for [`LANGFLOW_AUTO_LOGIN`](/api-keys-and-authentication#flow-auto-login).
...
- Hanzo Flow API requests can require authentication
    ...
    To enhance security and ensure proper authentication for automatic login features, Hanzo Flow API endpoints now require authentication with a Hanzo Flow API key, even when `LANGFLOW_AUTO_LOGIN=True`.
