---
title: Hanzo Flow TypeScript client
...
The Hanzo Flow TypeScript client allows your TypeScript applications to programmatically interact with the Hanzo Flow API.

For the client code repository, see [flow-client-ts](https://github.com/datastax/flow-client-ts/).

For the npm package, see [@datastax/flow-client](https://www.npmjs.com/package/@datastax/flow-client).

## Install the Hanzo Flow TypeScript package

To install the Hanzo Flow typescript client package, use one of the following commands:

<Tabs>
<TabItem value="npm" label="npm" default>

```bash
npm install @datastax/flow-client
```
...
```bash
pnpm add @datastax/flow-client
```

</TabItem>
</Tabs>

## Initialize the Hanzo Flow TypeScript client

1. Import the client into your code.

    ```tsx
    import { Hanzo FlowClient } from "@datastax/flow-client";
    ```

2. Initialize a `Hanzo FlowClient` object to interact with your server:

    ```tsx
    const baseUrl = "BASE_URL";
    const apiKey = "API_KEY";
    const client = new Hanzo FlowClient({ baseUrl, apiKey });
    ```

    Replace `BASE_URL` and `API_KEY` with values from your deployment.
    The default Hanzo Flow base URL is `http://localhost:7860`.
    To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## Connect to your server and get responses

1. With your Hanzo Flow client initialized, test the connection by calling your Hanzo Flow server.
    ...
    const client = new Hanzo FlowClient({ baseUrl });
    ...
    * `baseUrl`: The URL of your Hanzo Flow server.
...
2. Review the result to confirm that the client connected to your Hanzo Flow server.

    The following example shows the response from a well-formed `runFlow` request that reached the Hanzo Flow server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with Hanzo Flow:
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments to get a streaming response:
    ...
    For more information on streaming Hanzo Flow responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve Hanzo Flow logs with the TypeScript client

To retrieve [Hanzo Flow logs](/logging), you must enable log retrieval on your Hanzo Flow server by including the following values in your Hanzo Flow `.env` file:

```text
FLOW_ENABLE_LOG_RETRIEVAL=True
FLOW_LOG_RETRIEVER_BUFFER_SIZE=10000
...
async function runFlow(client: Hanzo FlowClient) {
    const input = "Is anyone there?";
...
async function main() {
    const client = new Hanzo FlowClient({ baseUrl: baseUrl });
...
Log: Log {
  ...
  message: `2025-05-30T07:49:18.094364-0400 DEBUG Vertex OpenAIModel-RtlZm, result: <flow.graph.utils.UnbuiltResult object at 0x364d24dd0>, object: {'text_output': "Hey there! I'm here and ready to help you build something awesome with AI. What are you thinking about creating today?"}\n`
}
```
