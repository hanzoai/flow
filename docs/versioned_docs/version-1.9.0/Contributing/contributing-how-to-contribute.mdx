---
title: Contribute to Hanzo Flow
...
This guide is intended to help you start contributing to Hanzo Flow.
As an open-source codebase in a rapidly developing field, Hanzo Flow welcomes contributions, whether it be in the form of a new feature, improved infrastructure, or better documentation.

To contribute code or documentation to Hanzo Flow, follow the [pull request guide](#open-a-pull-request).

## Hanzo Flow services overview

This overview will help you understand how to set up your development environment.

Hanzo Flow consists of two main services:
...
When you install Hanzo Flow as an end user, the backend serves the frontend internally, making both services available on a single port (`7860` by default).

## Install Hanzo Flow from source

Install Hanzo Flow from source by forking the repository and setting up your development environment.
...
### Clone the Hanzo Flow repository
...
```bash
git remote add FORK_NAME https://github.com/GIT_USERNAME/flow.git
```
Replace the following:
* `FORK_NAME`: A name for your fork of the repository
* `GIT_USERNAME`: Your Git username

### Run Hanzo Flow from source

You can run Hanzo Flow from source after cloning the repository, even if you aren't contributing to the codebase.
This builds the frontend and serves it through the backend on port `7860`.

The instructions below are for running Hanzo Flow from source. For development with hot reload, see [Set up development environment](#set-up-your-flow-development-environment).

<details>
<summary>Run from source on macOS/Linux</summary>

In your terminal, navigate to the root of the Hanzo Flow directory, and then run `make run_cli`.
...
The Hanzo Flow frontend is served at `http://localhost:7860`.
</details>

<details>
<summary>Run from source with Windows CMD</summary>

To run Hanzo Flow from source on Windows, you can use the Hanzo Flow repository's included scripts, or run the commands in the terminal.

Do one of the following:

* To install and run Hanzo Flow with the included Windows Batch file, navigate to the `scripts/windows` directory, and then run the `build_and_run.bat` file.

* To run Hanzo Flow from the Windows Command Line:
    ...
    2. Copy the contents of the built `src/frontend/build` directory to `src/backend/base/flow/frontend`.
...
* To install and run Hanzo Flow with the included scripts, navigate to the `scripts/windows` directory, and then run the `build_and_run.ps1` file.

* To run Hanzo Flow from a PowerShell terminal:
...
### Set up your Hanzo Flow development environment
...
If you just want to run Hanzo Flow locally without making code changes, see [Run Hanzo Flow from source](#run-flow-from-source).

<Tabs>
<TabItem value="linux-macos" label="Linux or macOS" default>

1. Set up the Hanzo Flow development environment:
...
Since Windows doesn't include `make`, building and running Hanzo Flow from source uses `npm` and `uv`.

To set up the Hanzo Flow development environment, run the frontend and backend in separate terminals:
...
2. Fork the [Hanzo Flow GitHub repository](https://github.com/hanzoai/flow).
...
4. From the root of your local Hanzo Flow fork, change to the `/docs` directory:
...
6. To edit and create content, work with the `.mdx` files in the `flow/docs/docs` directory.

    Create new files in `.mdx` format.

    Navigation is defined in `flow/docs/sidebars.js`.
...
* Pull request titles appear in Hanzo Flow's release notes, so they should explain what the PR does as explicitly as possible.
* Pull requests should strive to fix one thing **only**, and should contain a good description of what is being fixed.

3. A Hanzo Flow maintainer will review your pull request and may request changes, so ensure you pay attention to your PRs. Thanks for your contribution!
