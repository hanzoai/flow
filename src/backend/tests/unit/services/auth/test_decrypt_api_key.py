...
@pytest.fixture
def flow_auth_service(tmp_path):
    """Use Flow AuthService for encrypt/decrypt so tests get real Fernet behavior."""
...
def use_flow_auth_for_encryption(flow_auth_service):
    """Ensure utils use Flow AuthService (real encrypt/decrypt), not LFX stub."""
    with patch("flow.services.auth.utils.get_auth_service", return_value=flow_auth_service):
        yield
