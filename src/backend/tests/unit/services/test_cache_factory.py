...
def test_cache_type_disk_is_rejected(monkeypatch):
    """FLOW_CACHE_TYPE=disk must fail validation now that the disk backend is gone."""
    monkeypatch.setenv("FLOW_CACHE_TYPE", "disk")
...
def test_supported_cache_types_validate(monkeypatch, cache_type):
    monkeypatch.setenv("FLOW_CACHE_TYPE", cache_type)
    settings = Settings()
