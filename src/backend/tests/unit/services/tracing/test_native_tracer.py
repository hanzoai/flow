...
class TestIsEnabled:
    def test_enabled_by_default(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("FLOW_NATIVE_TRACING", None)
    ...
    def test_disabled_by_env_var(self, value):
        with patch.dict(os.environ, {"FLOW_NATIVE_TRACING": value}):
    ...
    def test_ready_property_reflects_is_enabled(self):
        with patch.dict(os.environ, {"FLOW_NATIVE_TRACING": "false"}):
            tracer = _make_tracer()
            assert tracer.ready is False

        with patch.dict(os.environ, {"FLOW_NATIVE_TRACING": "true"}):
            tracer = _make_tracer()
...
class TestFlushToDatabase:
    ...
    async def test_flush_error_status_when_span_has_error(self):
        ...
        from flow.services.database.models.traces.model import TraceTable
