...
from flow.services.tracing.service import (
    TracingService,
...
def mock_tracers():
    with (
        patch(
            "flow.services.tracing.service._get_langsmith_tracer",
            return_value=MockTracer,
        ),
        patch(
            "flow.services.tracing.service._get_langwatch_tracer",
            return_value=MockTracer,
        ),
        patch(
            "flow.services.tracing.service._get_langfuse_tracer",
            return_value=MockTracer,
        ),
        patch(
            "flow.services.tracing.service._get_arize_phoenix_tracer",
            return_value=MockTracer,
        ),
        patch(
            "flow.services.tracing.service._get_opik_tracer",
            return_value=MockTracer,
        ),
        patch(
            "flow.services.tracing.service._get_traceloop_tracer",
            return_value=MockTracer,
        ),
        patch(
...
async def test_start_tracers_with_exception(tracing_service):
    ...
    with (
        ...
        patch("flow.services.tracing.service.logger") as mock_logger,
    ):
...
async def test_trace_worker_with_exception(tracing_service):
    ...
    with patch("flow.services.tracing.service.logger") as mock_logger:
        # Configure async mock method
