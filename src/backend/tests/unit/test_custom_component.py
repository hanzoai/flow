...
from hanzoflow.custom import Component, CustomComponent
from hanzoflow.custom.code_parser.code_parser import CodeParser, CodeSyntaxError
from hanzoflow.custom.custom_component.base_component import BaseComponent, ComponentCodeNullError
...
def test_code_parser_parse_classes():
    ...
    parser = CodeParser("from hanzoflow.custom import Component\n\nclass Test(Component): pass")
    tree = parser.get_tree()
    for node in ast.walk(tree):
...
def test_custom_component_subclass_from_lctoolcomponent():
    ...
    code = dedent("""
    from hanzoflow.base.langchain_utilities.model import LCToolComponent
