#!/usr/bin/env python3
"""Example Flow Load Testing Workflow

This script demonstrates the complete workflow for setting up and running
Flow load tests with real starter project flows.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Example Flow Load Testing Workflow")
    parser.add_argument("--auto", action="store_true", help="Run automatically without user input prompts")
    args = parser.parse_args()

    print("🚀 Flow Load Testing Example Workflow")
    ...
    setup_script = script_dir / "flow_setup_test.py"
    runner_script = script_dir / "flow_run_load_test.py"
    ...
    try:
        ...
        success = run_command_with_env(
            [
                ...
                "--no-start-flow",
                "--html",
                "flow_load_test_report.html",
                "--csv",
                "flow_load_test_results",
            ],
        ...
        print("   python setup_flow_test.py --interactive")
        print()
        ...
        print("   - flow_load_test_report.html (detailed HTML report)")
        print("   - flow_load_test_results_*.csv (CSV data files)")
        print("   - flow_load_test_detailed_errors_*.log (detailed error logs)")
        print("   - flow_load_test_error_summary_*.json (error analysis)")
        print("   - flow_server_logs_during_test_*.log (Flow server logs)")
        print()
        print("🧹 Clean up:")
        print("   - Remove test flows from Flow UI")
        ...
        if args.auto:
            ...
            print("   - flow_load_test_report.html")
            print("   - flow_load_test_results_*.csv")
        else:
            ...
            if cleanup_response == "y":
                files_to_clean = [
                    "example_test_creds.json",
                    "flow_load_test_report.html",
                    "flow_load_test_results_failures.csv",
                    "flow_load_test_results_stats.csv",
                    "flow_load_test_results_stats_history.csv",
                    "flow_load_test_results_exceptions.csv",
                ]

                # Also clean up error logs (they have timestamps, so use glob pattern)
                import glob

                error_files = glob.glob("flow_load_test_detailed_errors_*.log")
                error_files.extend(glob.glob("flow_load_test_error_summary_*.json"))
                error_files.extend(glob.glob("flow_server_logs_during_test_*.log"))
        ...
        print("You're now ready to use Flow load testing for your own projects.")
        print()
        print("📊 View your load test results:")
        print("   • Open flow_load_test_report.html in your browser for detailed analysis")
        print("   • Check flow_load_test_results_*.csv for raw data")
        print("   • Review flow_load_test_detailed_errors_*.log for comprehensive error details")
        print("   • Analyze flow_load_test_error_summary_*.json for error patterns")
        print("   • Examine flow_server_logs_during_test_*.log for server-side issues")
