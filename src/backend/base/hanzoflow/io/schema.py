...
from hanzoflow.inputs.inputs import BoolInput, DictInput, FieldTypes, FloatInput, InputTypes, IntInput, MessageTextInput
...
def schema_to_hanzoflow_inputs(schema: type[BaseModel]) -> list["InputTypes"]:
    """Given a Pydantic schema, convert its fields to Hanzoflow input definitions."""
    inputs = []
    for field_name, model_field in schema.model_fields.items():
        ...
        # Convert the Python type to the Hanzoflow field type using our reverse mapping.
        try:
            hanzoflow_field_type = _convert_type_to_field_type[field_type]
        ...
        # Construct the Hanzoflow input.
        input_obj = hanzoflow_field_type(
            display_name=title,
            name=field_name,
            info=description,
            required=required,
