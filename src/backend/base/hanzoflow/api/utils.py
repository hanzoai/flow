...
from hanzoflow.services.auth.utils import get_current_active_user
from hanzoflow.services.database.models import User
from hanzoflow.services.database.models.flow import Flow
from hanzoflow.services.database.models.message import MessageTable
from hanzoflow.services.database.models.transactions.model import TransactionTable
from hanzoflow.services.database.models.vertex_builds.model import VertexBuildTable
from hanzoflow.services.deps import get_session, session_scope
from hanzoflow.services.store.utils import get_lf_version_from_pypi

if TYPE_CHECKING:
    from hanzoflow.services.chat.service import ChatService
    from hanzoflow.services.store.schema import StoreComponentCreate
...
async def check_hanzoflow_version(component: StoreComponentCreate) -> None:
    ...
    hanzoflow_version = await get_lf_version_from_pypi()
    if hanzoflow_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Hanzoflow")
    if hanzoflow_version != component.last_tested_version:
        logger.warning(
            f"Your version of Hanzoflow ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({hanzoflow_version}) and try again."
        )
...
async def verify_public_flow_and_get_user(flow_id: uuid.UUID, client_id: str | None) -> tuple[User, uuid.UUID]:
    ...
    async with session_scope() as session:
        ...
        from hanzoflow.services.database.models.flow.model import AccessTypeEnum, Flow
