...
from hanzoflow.initial_setup.setup import get_or_create_default_folder
from hanzoflow.logging.logger import configure, logger
from hanzoflow.main import setup_app
from hanzoflow.services.database.utils import session_getter
from hanzoflow.services.deps import get_db_service, get_settings_service, session_scope
from hanzoflow.services.settings.constants import DEFAULT_SUPERUSER
from hanzoflow.services.utils import initialize_services
from hanzoflow.utils.version import fetch_latest_version, get_version_info
...
def run(
    *,
    ...
    for key, value in os.environ.items():
        new_key = key.replace("HANZOFLOW_", "")
...
def run_on_mac_or_linux(host, port, log_level, options, app):
    webapp_process = Process(target=run_hanzoflow, args=(host, port, log_level, options, app))
...
def run_on_windows(host, port, log_level, options, app) -> None:
    """Run the Hanzoflow server on Windows."""
    print_banner(host, port)
    run_hanzoflow(host, port, log_level, options, app)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "hanzoflow")
        'A new version of hanzoflow is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=hanzoflow_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if hanzoflow_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int) -> None:
    ...
    hanzoflow_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= hanzoflow_is_pre_release(hanzoflow_version)  # Update pre-release status

    notice = build_version_notice(hanzoflow_version, package_name)
    ...
    info_text = (
        ...
        "[bold][link=https://github.com/hanzoflow-ai/hanzoflow]GitHub Repo[/link][/bold] :star2:"
    )
    telemetry_text = (
...
def run_hanzoflow(host, port, log_level, options, app) -> None:
    """Run Hanzoflow server on localhost."""
    if platform.system() == "Windows":
        import uvicorn
        ...
        from hanzoflow.server import HanzoflowApplication

        server = HanzoflowApplication(app, options)
...
def superuser(
    ...
    log_level: str = typer.Option("error", help="Logging level.", envvar="HANZOFLOW_LOG_LEVEL"),
) -> None:
    """Create a superuser."""
    ...
    async def _create_superuser():
        ...
        async with session_getter(db_service) as session:
            from hanzoflow.services.auth.utils import create_super_user

            if await create_super_user(db=session, username=username, password=password):
                # Verify that the superuser was created
                from hanzoflow.services.database.models.user.model import User
...
def copy_db() -> None:
    ...
    This function copies the 'hanzoflow.db' and 'hanzoflow-pre.db' files from the cache directory to the current
    directory.
    ...
    db_path = cache_dir / "hanzoflow.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from hanzoflow.services.database.models.api_key import ApiKey, ApiKeyCreate
            from hanzoflow.services.database.models.api_key.crud import create_api_key, delete_api_key
