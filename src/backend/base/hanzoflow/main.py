...
from hanzoflow.api import health_check_router, log_router, router
...
from hanzoflow.interface.utils import setup_llm_caching
from hanzoflow.logging.logger import configure
from hanzoflow.middleware import ContentSizeLimitMiddleware
from hanzoflow.services.deps import (
    get_queue_service,
    get_settings_service,
    get_telemetry_service,
)
from hanzoflow.services.utils import initialize_services, teardown_services
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            logger.debug(f"Starting Hanzoflow v{version}...")
        else:
            logger.debug("Starting Hanzoflow...")
        ...
        try:
            ...
            # Final message
            logger.debug("Hanzoflow shutdown complete")
...
def create_app():
    ...
    app = FastAPI(lifespan=lifespan, title="Hanzoflow", version=__version__)
    app.add_middleware(
    ...
    if prome_port_str := os.environ.get("HANZOFLOW_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from hanzoflow.api.v1 import mcp_router
...
def get_static_files_dir():
    """Get the static files directory relative to Hanzoflow's main.py file."""
...
if __name__ == "__main__":
    ...
    from hanzoflow.__main__ import get_number_of_workers

    configure()
    uvicorn.run(
        "hanzoflow.main:create_app",
        host="127.0.0.1",
        port=7860,
