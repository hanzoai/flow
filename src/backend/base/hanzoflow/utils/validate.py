...
from hanzoflow.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
...
def create_class(code, class_name):
    ...
    # Replace from hanzoflow import CustomComponent with from hanzoflow.custom import CustomComponent
    code = code.replace("from hanzoflow import CustomComponent", "from hanzoflow.custom import CustomComponent")
    code = code.replace(
...
def get_default_imports(code_string):
    ...
    hanzoflow_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, hanzoflow_imports)
    hanzoflow_module = importlib.import_module("hanzoflow.field_typing")
    default_imports.update({name: getattr(hanzoflow_module, name) for name in necessary_imports})
