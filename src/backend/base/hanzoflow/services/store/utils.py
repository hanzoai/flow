...
if TYPE_CHECKING:
    from hanzoflow.services.store.schema import ListComponentResponse
...
async def get_lf_version_from_pypi():
    try:
        ...
        logger.opt(exception=True).debug("Error getting the latest version of hanzoflow from PyPI")
        return None
