...
from hanzoflow.services.settings.constants import VARIABLES_TO_GET_FROM_ENVIRONMENT
from hanzoflow.utils.util_strings import is_valid_database_url
...
class Settings(BaseSettings):
    # Define the default HANZOFLOW_DIR
    config_dir: str | None = None
    # Define if hanzoflow db should be saved in config dir or
    # in the hanzoflow directory
    save_db_in_config_dir: bool = False
    """Define if hanzoflow database should be saved in HANZOFLOW_CONFIG_DIR or in the hanzoflow directory
    (i.e. in the package directory)."""

    dev: bool = False
    """If True, Hanzoflow will run in development mode."""
    database_url: str | None = None
    """Database URL for Hanzoflow. If not provided, Hanzoflow will use a SQLite database.
    ...
    """If True, Hanzoflow will retry to connect to the database if it fails."""
    pool_size: int = 20
    ...
    """If set to True, Hanzoflow will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which Hanzoflow will expose Prometheus metrics. 9090 is the default port."""
    ...
    store_url: str | None = "https://api.hanzoflow.store"
    download_webhook_url: str | None = "https://api.hanzoflow.store/flows/trigger/ec611a61-8460-4438-b187-a4f65e5559d4"
    like_webhook_url: str | None = "https://api.hanzoflow.store/flows/trigger/64275852-ec00-45c1-984e-3bff814732da"
    ...
    with the same name in case Hanzoflow fails to retrieve the variable value."""
    ...
    user_agent: str = "hanzoflow"
    """User agent for the API calls."""
    backend_only: bool = False
    """If set to True, Hanzoflow will not serve the frontend."""

    # Telemetry
    do_not_track: bool = True
    """If set to True, Hanzoflow will not track telemetry."""
    telemetry_base_url: str = "https://hanzoflow.gateway.scarf.sh"
    transactions_storage_enabled: bool = True
    """If set to True, Hanzoflow will track transactions between flows."""
    vertex_builds_storage_enabled: bool = True
    """If set to True, Hanzoflow will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "127.0.0.1"
    """The host on which Hanzoflow will run."""
    port: int = 7860
    """The port on which Hanzoflow will run."""
    workers: int = 1
    """The number of workers to run."""
    log_level: str = "critical"
    """The log level for Hanzoflow."""
    log_file: str | None = "logs/hanzoflow.log"
    """The path to log file for Hanzoflow."""
    ...
    """If set to True, Hanzoflow will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, Hanzoflow will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which Hanzoflow will auto save flows."""
    ...
    """If set to False, Hanzoflow will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, Hanzoflow will not send progress notifications in the MCP server."""
    ...
    """If set to True, Hanzoflow will only partially load components at startup and fully load them on demand.
    This significantly reduces startup time but may cause a slight delay when a component is first used."""

    @field_validator("dev")
    @classmethod
    def set_dev(cls, value):
        from hanzoflow.settings import set_dev
    ...
    @classmethod
    def set_hanzoflow_dir(cls, value):
        if not value:
            from platformdirs import user_cache_dir

            # Define the app name and author
            app_name = "hanzoflow"
            app_author = "hanzoflow"

            # Get the cache directory for the application
            cache_dir = user_cache_dir(app_name, app_author)

            # Create a .hanzoflow directory inside the cache directory
    ...
    def set_database_url(cls, value, info):
        ...
        logger.debug("No database_url provided, trying HANZOFLOW_DATABASE_URL env variable")
        if hanzoflow_database_url := os.getenv("HANZOFLOW_DATABASE_URL"):
            value = hanzoflow_database_url
            logger.debug("Using HANZOFLOW_DATABASE_URL env variable.")
        else:
            logger.debug("No database_url env variable, using sqlite database")
            # Originally, we used sqlite:///./hanzoflow.db
            ...
            from hanzoflow.utils.version import is_pre_release as hanzoflow_is_pre_release

            version = get_version_info()["version"]
            is_pre_release = hanzoflow_is_pre_release(version)
            ...
            if info.data["save_db_in_config_dir"]:
                ...
                logger.debug(f"Saving database to hanzoflow directory: {database_dir}")

            pre_db_file_name = "hanzoflow-pre.db"
            db_file_name = "hanzoflow.db"
    ...
    def set_components_path(cls, value):
        if os.getenv("HANZOFLOW_COMPONENTS_PATH"):
            logger.debug("Adding HANZOFLOW_COMPONENTS_PATH to components_path")
            hanzoflow_component_path = os.getenv("HANZOFLOW_COMPONENTS_PATH")
            if Path(hanzoflow_component_path).exists() and hanzoflow_component_path not in value:
                if isinstance(hanzoflow_component_path, list):
                    for path in hanzoflow_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {hanzoflow_component_path} to components_path")
                elif hanzoflow_component_path not in value:
                    value.append(hanzoflow_component_path)
                    logger.debug(f"Appending {hanzoflow_component_path} to components_path")
    ...
    model_config = SettingsConfigDict(validate_assignment=True, extra="ignore", env_prefix="HANZOFLOW_")
