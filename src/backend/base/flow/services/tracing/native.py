...
execution traces directly in Flow's database, enabling the Trace View
...
class NativeTracer(BaseTracer):
    """Tracer that stores execution traces in Flow's database.

    This tracer captures:
    - Component-level traces (via add_trace/end_trace)
    - LangChain-level traces (via get_langchain_callback)

    Enabled by default. Disable with FLOW_NATIVE_TRACING=false if needed.
    """
    ...
    def _is_enabled() -> bool:
        ...
        return os.getenv("FLOW_NATIVE_TRACING", "true").lower() not in ("false", "0", "no")
    ...
    async def _flush_to_database(self, error: Exception | None = None) -> None:
        ...
        try:
            ...
            from flow.services.tracing.formatting import safe_int_tokens
    ...
    def get_langchain_callback(self) -> BaseCallbackHandler | None:
        ...
        from flow.services.tracing.native_callback import NativeCallbackHandler
        from flow.services.tracing.service import component_context_var
        ...
        if component_context:
            ...
            parent_span_id = uuid5(FLOW_SPAN_NAMESPACE, f"{self.trace_id}-{component_id}")
        elif self._current_component_id:
            # Fallback for edge cases where component context might not be set
            parent_span_id = uuid5(FLOW_SPAN_NAMESPACE, f"{self.trace_id}-{self._current_component_id}")
    ...
    def end_langchain_span(
        ...
        # Default to chat since most LLM usage in Flow is conversational
    ...
    def _map_trace_type(trace_type: str) -> SpanType:
        """Normalise Flow's string trace types to the SpanType enum, defaulting to CHAIN for unknown values."""
