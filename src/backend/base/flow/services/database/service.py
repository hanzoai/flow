...
from flow.services.database import models
from flow.services.database.constants import (
...
from flow.services.database.session import NoopSession
from flow.services.database.utils import Result, TableResults
...
class DatabaseService(Service):
    ...
    def __init__(self, settings_service: SettingsService):
        ...
        if self.alembic_log_to_stdout:
            ...
            self.alembic_log_path = Path(flow_dir) / alembic_log_file
    ...
    async def ensure_postgresql_version(self) -> None:
        ...
        Flow's schema uses UNIQUE NULLS DISTINCT, which is only supported in PostgreSQL 15+.
