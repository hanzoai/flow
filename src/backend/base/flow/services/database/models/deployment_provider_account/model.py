...
class DeploymentProviderAccount(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    # by a Flow alembic migration.  The enum is Flow-owned (see
    ...
    #   2. Register the corresponding adapter in LFX and mapper in Flow.
    #   3. Create a Flow alembic migration that runs:
    #        op.execute(
