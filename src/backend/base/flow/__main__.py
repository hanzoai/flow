...
from flow.cli.progress import create_flow_progress
...
from flow.utils.version import is_pre_release as flow_is_pre_release
...
try:
    ...
    lfx_app = typer.Typer(name="lfx", help="Flow Executor commands")
...
def run(
    *,
    ...
    progress = create_flow_progress(verbose=verbose)

    # Step 0: Initializing Flow
    ...
    if platform.system() == "Windows":
        ...
        with progress.step(6):
            # Use Gunicorn with FlowUvicornWorker for non-Windows systems
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    with suppress(httpx.ConnectError):
        ...
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if flow_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    is_pre_release |= flow_is_pre_release(flow_version)  # Update pre-release status

    notice = build_version_notice(flow_version, package_name)
    ...
    access_link = f"[bold]{status_icon} Open Flow {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    from flow.services.database.models.user.crud import get_all_superusers
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from flow.services.database.models.api_key.crud import create_api_key, delete_api_key
