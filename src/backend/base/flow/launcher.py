...
def main():
    """Launches flow with appropriate environment setup.
...
def _launch_with_exec():
    """Launch flow by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "flow.__main__", *sys.argv[1:]])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec flow: {e}", file=sys.stderr)
        sys.exit(1)
