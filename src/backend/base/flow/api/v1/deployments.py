...
DeploymentProviderAccountIdQuery = Annotated[
    UUID,
    Query(description="Flow DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentProviderAccountIdPath = Annotated[
    UUID,
    Path(description="Flow DB provider-account UUID (`deployment_provider_account.id`)."),
]
DeploymentIdPath = Annotated[
    UUID,
    Path(description="Flow DB deployment UUID (`deployment.id`)."),
]
...
DeploymentIdQuery = Annotated[
    UUID,
    Query(description="Flow DB deployment UUID (`deployment.id`)."),
]
...
IncludeProviderDeleteQuery = Annotated[
    ...
    Query(
        description=(
            ...
            "When false, only the local Flow DB row is removed; "
...
async def _delete_local_deployment_row_with_commit_retry(
    *,
    ...
    stale Flow row that still blocks later reads or provider-account
    deletion.
    """
    try:
...
async def create_deployment(
    ...
    if existing_resource_key is not None:
        ...
        if existing_deployment is not None:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"The agent '{existing_resource_key}' is already managed by Flow. "
...
async def list_deployments(
    ...
    *,
    load_from_provider: Annotated[
        bool,
        Query(
            description=("When true, list deployments directly from the provider (bypassing Flow deployment rows).")
        ),
    ] = False,
    flow_version_ids: Annotated[
        ...
        Query(
            description=(
                "Optional Flow flow version ids (pass as repeated query params, "
...
async def update_snapshot(
    ...
    *,
    ...
    ``provider_snapshot_id``.  Only Flow-tracked snapshots (those with
