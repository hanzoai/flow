...
from flow.api.v1.chat import build_flow_and_stream
from flow.memory import aadd_messagetables
from flow.schema.properties import Properties
from flow.services.auth.utils import get_current_user_for_websocket
...
from flow.services.deps import get_variable_service, session_scope
from flow.utils.voice_utils import BYTES_PER_24K_FRAME, VAD_SAMPLE_RATE_16K, resample_24k_to_16k
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "flow.voice_mode.config":
                            await logger.ainfo(f"flow.voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "flow.elevenlabs.config":
                            await logger.ainfo(f"flow.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "flow.elevenlabs.config":
                            await logger.ainfo(f"flow.elevenlabs.config {event}")
