...
from flow.api.utils.mcp import (
...
from flow.api.v1.mcp import ResponseNoOp
from flow.api.v1.mcp_utils import (
...
from flow.api.v1.schemas import (
    AuthSettings,
    ComposerUrlResponse,
    MCPInstallRequest,
    MCPProjectResponse,
...
from flow.services.auth.constants import AUTO_LOGIN_WARNING
from flow.services.auth.mcp_encryption import decrypt_auth_settings, encrypt_auth_settings
from flow.services.database.models import Flow, Folder
from flow.services.database.models.api_key.crud import check_key, create_api_key
from flow.services.database.models.api_key.model import ApiKey, ApiKeyCreate
from flow.services.database.models.user.crud import get_user_by_username
...
async def verify_project_auth_conditional(
    ...
    async with session_scope() as session:
        ...
        from flow.services.auth.utils import get_current_user_mcp
...
def get_project_sse(project_id: UUID | None) -> SseServerTransport:
    ...
    if project_id_str not in project_sse_transports:
        project_sse_transports[project_id_str] = SseServerTransport(f"/v1/mcp/project/{project_id_str}/")
...
async def install_mcp_config(
    ...
    try:
        ...
        if use_mcp_composer:
            ...
            command = "uvx"
            args = ["mcp-proxy"]
            # Check if we need to add Flow API key headers
            # Necessary only when Project API Key Authentication is enabled

            # Generate a Flow API key for auto-install if needed
            ...
            if should_generate_api_key:
                async with session_scope() as api_key_session:
                    ...
                    flow_api_key = api_key_response.api_key
                    args.extend(["--headers", "x-api-key", flow_api_key])
        ...
        # Remove stale entries that point to the same Flow URLs (e.g. after the project is renamed)
...
async def register_project_with_composer(project: Folder):
    ...
    try:
        ...
        if not settings.host or not settings.port:
            error_msg = "Flow host and port must be set in settings to register project with MCP Composer"
            raise ValueError(error_msg)
