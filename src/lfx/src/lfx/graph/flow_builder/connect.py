...
# Flow uses oe (U+0153) as a quote replacement in ReactFlow handle strings
_Q = "\u0153"


def _scaped_json_stringify(obj: Any) -> str:
    """Replicate Flow frontend's scapedJSONStringfy: sorted keys, compact, oe for quotes."""
    return _custom_stringify(obj).replace('"', _Q)


def _custom_stringify(obj: Any) -> str:
    """Replicate Flow frontend's customStringify for edge handle data.
...
def add_connection(
    ...
    # of by edge id, since UI-saved edges from older Flow versions use a
