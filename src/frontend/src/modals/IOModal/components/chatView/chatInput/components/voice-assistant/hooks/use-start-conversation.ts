...
export const useStartConversation = (
  ...
  const url = `${protocol}//${currentHost}:${currentPort}/v1/voice/ws/flow_tts/${flowId}/${currentSessionId?.toString()}`;
  //const url = `${protocol}//${currentHost}:${currentPort}/v1/voice/ws/flow_as_tool/${flowId}/${currentSessionId?.toString()}`;
  ...
  try {
    ...
    wsRef.current.onopen = () => {
      ...
      if (wsRef.current?.readyState === WebSocket.OPEN) {
        wsRef.current.send(
          JSON.stringify({
            type: "hanzoflow.elevenlabs.config",
