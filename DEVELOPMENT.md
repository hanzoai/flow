...
You will push changes to a fork of the Hanzoflow repository, and from there create a Pull Request into the project repository.

Fork the [Hanzoflow GitHub repository](https://github.com/hanzoflow-ai/hanzoflow/fork), and follow the instructions to create a new fork.
...
```bash
git clone https://github.com/<your username>/hanzoflow.git
```
...
```bash
cd hanzoflow
git remote add upstream https://github.com/hanzoflow-ai/hanzoflow.git
...
This will set up the development environment by installing backend and frontend dependencies, building the frontend static files, and initializing the project. It runs `make install_backend`, `make install_frontend`, `make build_frontend`, and finally `uv run hanzoflow run` to start the application.
...
However, as a developer,  you will want to proceed to the next step. Shutdown Hanzoflow by hitting `Control (or Command)-C`.
...
## Run Hanzoflow in "Development" Mode
...
```
INFO:     Will watch for changes in these directories: ['/home/phil/git/hanzoflow']
...
The frontend (User Interface) is, in shipped code (i.e. via `hanzoflow run`), statically-compiled files that the backend FastAPI service provides to clients via port `7860`. In development mode, these are served by a Node.js service on port `3000`. In the *Frontend Terminal*, start the frontend service:
...
At this point, you can navigate to http://localhost:3000/ in a browser and access the Hanzoflow User Interface.
...
Components reside in folders under `src/backend/base/hanzoflow`, and their unit tests under `src/backend/base/tests/unit/components`. 
...
As a final validation, stop the backend and frontend services and run `make init`; this will do a clean build and the UI should be available in port `7860` (as it has invoked `hanzoflow run`). Open a **new** browser tab to this  service and do a final check of your changes by adding your new/modified component onto the canvas from the Components list.
...
* Files in `src/backend/base/hanzoflow/initial_setup/starter_projects` modify after `hanzoflow run`; these are formatting changes. Feel free to commit (or ignore) them.
