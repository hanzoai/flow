...
2. **Scans backend files**: Finds all `.py` files in `src/backend/base/flow/components/` with `legacy = True`
...
cd src/backend && python -m pytest --cov=src/backend/base/flow --cov-config=.coveragerc
```
...
- **Input**: `src/backend/base/flow/components/**/*.py` (legacy components)
