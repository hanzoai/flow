...
- user.store_api_key: Flow Store API keys
...
def get_default_config_dir() -> Path:
    """Get the default Flow config directory using platformdirs."""
    return Path(user_cache_dir("flow", "flow"))


def get_config_dir() -> Path:
    """Get the Flow config directory from environment or default."""
...
def ensure_valid_key(s: str) -> bytes:
    ...
    to keep the migration script self-contained (can run without full Flow installation).
...
def migrate(
    ...
    *,
    ...
    Args:
        config_dir: Path to Flow config directory containing secret_key file.
    ...
    if dry_run:
        ...
        print("\nNext steps:")
        print("1. Start Flow and verify everything works")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate Flow encrypted data to a new secret key",
...
Examples:
  ...
  %(prog)s --old-key "current-key" --new-key "replacement-key"
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=f"Flow config directory (default: {default_config})",
    )
    parser.add_argument(
        "--database-url",
        type=str,
